	...
	*/
	...
	private function createRender($first_time=true) {
		...
		if ($this->style == Authentication::STYLE_2_LINES) {
			...
			$row = new RowTable();
			$row->add(new WSPObject($this->login_label, ":&nbsp;", $this->login->setFocus()))->add("&nbsp;") 
						->add(new WSPObject($this->password_label, ":&nbsp;", $this->password))->add("&nbsp;");
			for ($i=0; $i < sizeof($this->custom_field_array); $i++) {
               $row->add(new WSPObject($this->custom_label_array[$i], ":&nbsp;"))->add($this->custom_field_array[$i])->add("&nbsp;");
            }
		...
		$this->form->setContent(new WSPObject($this->table_main, $this->hdnReferer));
